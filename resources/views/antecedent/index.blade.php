            ...
            <div class="card">
                <div class="card-header">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        @can('crear-antecedente')

                            <a class="text-white mr-3 mt-3 btn btn-primary" data-toggle="modal" data-target="#exampleModal" data-modal-origin="create">Nuevo
                    ...
                    <div class="table-responsive">
                        <table id="datatable" class="table is-striped" style="width:100%">
                            <thead class="thead-light">
                                <tr>
                                    <th>Numero</th>                                        
                                    <th>Descripcion del antecedente</th>
                            ...
                            <tbody>
                                ...
                                <tr>
									...
									<td>{{ $antecedent->File->file_number}}</td>
                                    <!-- Modal editar -->
                                    <div class="modal fade" id="editarmodal{{$antecedent->id}}" tabindex="-1" role="dialog" aria-labelledby="editarmodalLabel" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                @can('crear-antecedente')

                                                <div class="modal-header">Editar Antecedente
                                                ...
                                                @endcan
                                                    <div class="modal-body">
                                                        <form  action="{{ route ('antecedents.update', $antecedent->id) }}" method="POST" enctype="multipart/form-data">
                                                            @csrf
                                                            @method('PUT')
                                                            <div class="mb-3">
                                                                <label for="antecedent_description" class="col-form-label">Descripcion del antecedente</label>
                                                                <input id="antecedent_description" type="text" class="form-control input-redondeado @error('antecedent_description') is-invalid @enderror" name="antecedent_description" value="{{ $antecedent->antecedent_description }}">
                                                                @error('antecedent_description')
                                                            ...
                                                            <div class="mb-3">
                                                                <label for="file_id" class="col-form-label">Numero de expediente</label>
                                                                <select name="file_id" id="file_id" class="form-control input-redondeado" required>
                                                                    <option value="" disabled>Seleccionar</option>
                                                                    @foreach($files as $file)
                                                                        <option value="{{$file->id}}" {{ $file->id == $antecedent->file_id ? 'selected' : '' }}>{{$file->file_number}}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                    ...
                                    <td>
                                        <form class="Form-Delete" action="{{ route('antecedents.destroy',$antecedent->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            @can('editar-antecedente')

                                            <button type="button" class="btn btn-primary " data-toggle="modal" data-target="#editarmodal{{$antecedent->id}}" style="text-align: center; width: 100px; height: 40px;">Editar
                                            ...
                                            @endcan
                                            @can('borrar-antecedente')

                                            <button type="submit" class="Form-Delete btn btn-danger" style="text-align: center; width: 100px; height: 40px;">Eliminar
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Crear Antecedente</h5>
                <button type="button" class="btn-close btn-close-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('antecedents.store') }}" method="POST" enctype="multipart/form-data">
                    {{@csrf_field()}}
                    <div class="mb-3">
                        <label for="antecedent_description" class="col-form-label">Descripcion</label>
                        <input id="antecedent_description" type="text" class="form-control input-redondeado @error('antecedent_description') is-invalid @enderror" name="antecedent_description" value="{{old ('antecedent_description') }}" autocomplete="off" autofocus>
                        @error('antecedent_description')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="mb-3">
                        <label for="file_id" class="col-form-label">Numero de expediente</label>
                        <select name="file_id" id="file_id" class="form-control input-redondeado" require><option value="" selected disabled>Seleccionar</option>
                            @foreach($files as $file)
                                <option value="{{$file->id}}">Numero de expediente: {{$file->file_number}}</option>
                            @endforeach
                        </select>                       
                    </div>
                    <div class="modal-footer">
