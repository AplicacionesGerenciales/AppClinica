        ...
        <div class="row">
            ...
            <thead class=" thead-light">
                <tr>
                    ...
                    <th scope="col" class="color-text">Descripcion</th>
                    <th scope="col" class="color-text">Opciones</th>
                </tr>
            </thead>
            <tbody>
                @foreach($diseaseGroups as $enfermedadesClasifi)
                <tr>
                    <td>{{$enfermedadesClasifi->id}}</td>
                    <td>{{$enfermedadesClasifi->disease_group_name}}</td>
                    <td>{{$enfermedadesClasifi->description}}</td>
                    <td>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action ="{{ route ('disease-groups.update', $enfermedadesClasifi->id )}}" method = "post">
                                            @csrf
                                            @method('PUT')
                                            <div class="mb-3">
                                                <label for="disease_group_name" class="col-form-label">Nombre</label>
                                                <input id="disease_group_name" type="text" value="{{ $enfermedadesClasifi->disease_group_name }}" class="form-control input-redondeado @error('disease_group_name') is-invalid @enderror" name="disease_group_name"  autocomplete="off" autofocus>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input id="Description" type="text" value="{{ $enfermedadesClasifi->description }}" class="form-control input-redondeado" name="description">
                                            </div>
                        ...
                        <form class="Form-Delete" action = "{{ route ('disease-groups.destroy', $enfermedadesClasifi->id )}}" method = "POST">
                            @csrf
                            @method('DELETE')
                            @can('editar-grupo-enfermedad')

                            <a class = "btn btn-primary" data-toggle="modal" data-target="#UpdateModal{{$enfermedadesClasifi->id}}" style="background-color:#01499B; color:white;">Editar
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                {{@csrf_field()}}
                    <div class="mb-3">
                        <label for="disease_group_name" class="col-form-label">Nombre</label>
                        <input id="disease_group_name" type="text" class="form-control input-redondeado @error('disease_group_name') is-invalid @enderror" name="disease_group_name" value="{{old ('disease_group_name') }}" autocomplete="off" autofocus>
                        @error('disease_group_name')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
