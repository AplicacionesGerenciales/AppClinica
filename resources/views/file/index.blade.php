                                ...
                                <tbody>
                                    @foreach ($files as $file)
                                        <tr>
                                            ...
                                            <td>
                                                <!-- Modal editar -->
                                        <div class="modal fade" id="editarmodal{{$file->id}}" tabindex="-1" role="dialog" aria-labelledby="editarmodalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">Editar Expediente
                                                    ...
                                                    <div class="modal-body">
                                                        <form  action="{{ route ('files.update', $file->id) }}" method="POST" enctype="multipart/form-data">
                                                            ...
                                                            @method('PUT')
                                                            <div class="mb-3">
                                                                <label for="file_number" class="col-form-label">Descripcion</label>
                                                                <input id="file_number" type="text" class="form-control input-redondeado @error('file_number') is-invalid @enderror" name="file_number" value="{{ $file->file_number }}">
                                                                    @error('file_number')
                                                            ...
                                                            <div class="mb-3">
                                                                    ...
                                                                    <input id="message" type="text" class="form-control input-redondeado @error('message') is-invalid @enderror" name="message" value="{{ $file->message }}">
                                                                    @error('message')
                                                                        <span class="invalid-feedback" role="alert">
                                                                            <strong>{{ $message }}</strong>
                                                                        </span>
                                                            ...
                                                            <!-- <div class="mb-3">
                                                                ...
                                                                <select name="patient_id" id="patient_id" class="form-control input-redondeado" require>
                                                                    <option value="" selected disabled>Seleccionar</option>
                                                                    @foreach($patients as $patiente)
                                                                    <option value="{{$patiente->id}}" {{$patiente->id == $file->patient_id ? 'selected' : ''}}>{{$patiente->name}}</option>
                                                                    @endforeach
                                                                </select>  
                                                            </div> -->
                                                            <div class="mb-3">
                                            ...
                                            <form class="Form-Delete" action="{{ route('files.destroy',$file->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                                <a class="btn btn-sm btn-primary " data-toggle="modal" data-target="#editarmodal{{$file->id}}"><i class="fa fa-fw fa-eye"></i> {{ __('Editar') }}
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            {{@csrf_field()}}
                <div class="mb-3">
                    <label for="file_number" class="col-form-label">Numero de expediente</label>
                    <input id="file_number" type="text" class="form-control input-redondeado @error('file_number') is-invalid @enderror" name="file_number" value="{{old ('file_number') }}" autocomplete="off" autofocus>
                        @error('file_number')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="mb-3">
                        <label for="message" class="col-form-label">Mensaje</label>
                        <input id="message" type="text" class="form-control input-redondeado @error('message') is-invalid @enderror" name="message" value="{{old ('message') }}" autocomplete="off" autofocus>
                        @error('message')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
