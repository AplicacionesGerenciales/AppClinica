...
<div class="card shadow">
    <div class="row align-items-centery">
        <div class="col">
            <h3 class="color-text mt-3 ml-3">Lista de Tipo de examenes</h3>
        </div>
        <div class="col text-right">
            @can('crear-tipo-examen')
        ...
        <div class="table-responsive">
            <table id="datatable" class="table is-striped" style="width:100%">
                <thead class=" thead-light">
                    <tr>
                        <th scope="col" class="color-text">Tipo de examen</th>
                        <th scope="col" class="color-text">Descripción</th>
                        <th scope="col" class="color-text">Opciones</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($typeExaminations as $tipoexamen)
                    <tr>
                        <td>{{$tipoexamen->name}}</td>
                        <td>{{$tipoexamen->description}}</td>
                        <td>
                            <div class="modal fade" id="UpdateModal{{$tipoexamen->id}}" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="UpdateModalLabel" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">Editar tipo-examen
                                            <button type="button" class="btn-close btn-close-white" data-dismiss="modal" aria-label="Close">
                                            </button>
                                        </div>
                                        <div class="modal-body">
                                            <form action ="{{ route ('type-examinations.update', $tipoexamen->id )}}" method = "post">
                                                @csrf
                                                @method('PUT')
                                                <div class="mb-3">
                                                    <label for="name" class="col-form-label">Tipo de examen</label>
                                                    <input id="name" type="text" value="{{ $tipoexamen->name }}" class="form-control input-redondeado @error('name') is-invalid @enderror" name="name"  autocomplete="off" autofocus>
                                                    @error('name')
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input id="Description" type="text" value="{{ $tipoexamen->description }}"  class="form-control input-redondeado" name="description">
                                                </div>
                            ...
                            <form class="Form-Delete" action = "{{ route ('type-examinations.destroy', $tipoexamen->id )}}" method = "POST">
                                @csrf
                                @method('DELETE')
                                @can('editar-tipo-examen')
                                
                                <a class = "btn btn-sm btn-primary" data-toggle="modal" data-target="#UpdateModal{{$tipoexamen->id}}" style="background-color:#01499B; color:white;">Editar
...
<div class="modal fade" id="CreateModal" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="CreateModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">Crear tipo-examen
                <button type="button" class="btn-close btn-close-white" data-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action ="{{ route('type-examinations.store') }}" method = "POST" enctype="multipart/form-data">
                {{@csrf_field()}}
                    <div class="mb-3">
                        <label for="name" class="col-form-label">tipo-examen</label>
